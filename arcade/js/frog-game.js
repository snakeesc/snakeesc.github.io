...
// --------------------------------------------------
  ...
  hud.className = "arc-hud arc-hud-main";
  const timerLabel = document.createElement("span");
  timerLabel.className = "arc-hud-stat arc-hud-stat-time";
  const frogsLabel = document.createElement("span");
  frogsLabel.className = "arc-hud-stat arc-hud-stat-frogs";
  const scoreLabel = document.createElement("span");
  scoreLabel.className = "arc-hud-stat arc-hud-stat-score";
  const scoreSep = document.createElement("span");
  scoreSep.className = "arc-hud-divider";
  ...
  miniBoard.className = "arc-hud arc-hud-mini-board";
  ...
  statsPanel.className = "arc-hud arc-hud-stats-panel";
  container.appendChild(statsPanel);

  // Controls — top left
  const controlsBar = document.createElement("div");
  controlsBar.className = "arc-hud arc-hud-controls";

  function makeControlButton(label) {
    const btn = document.createElement("button");
    btn.textContent = label;
    btn.className = "arc-hud-btn";
    return btn;
  }
  ...
  btnEnd.classList.add("arc-hud-btn-danger");
  ...
  gameOverBanner.className = "arc-game-over-banner";
  ...
  function updateStatsPanel() {
    ...
    const icon = (emoji, label, val, color) => {
      ...
      <div class="arc-stat-chip arc-stat-cat-${cat}">
        ...
        <span class="arc-stat-chip-label">${label}${val ? ` <strong class="arc-stat-chip-val">${val}</strong>` : ""}</span>
      </div>`;
    };
    ...
    if (snakeEggActive) {
      ...
      items.push(
        icon("🥚", "Next hatch roll", `${Math.ceil(remaining)}s`, o)
      );
    }
    if (babySnakes.length > 0) {
      ...
      items.push(icon("🐍", "Baby snakes", `${remaining.toFixed(0)}s`, r));
    }
