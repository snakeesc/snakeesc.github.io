...
(function () {
  ...
  */
  ...
  const NOMINAL_STEP_DISTANCE = SNAKE_BASE_SPEED / 60; // ~60 FPS baseline
  const MIN_SEGMENT_GAP = 6;
  const MAX_SEGMENT_GAP = 120;
  let smoothedSegmentGap = null;
  let lastStepDistance = null;

  function computeSegmentGap(stepDistance) {
    // Keep spacing stable across devices and frame rates so the body never stretches apart.
    const step = Math.max(stepDistance || lastStepDistance || NOMINAL_STEP_DISTANCE, 0.1);
    const targetGap = TARGET_SEGMENT_SPACING / step;
    const clampedGap = Math.max(MIN_SEGMENT_GAP, Math.min(MAX_SEGMENT_GAP, targetGap));

    if (smoothedSegmentGap === null) {
      smoothedSegmentGap = clampedGap;
    } else {
      smoothedSegmentGap = smoothedSegmentGap * 0.85 + clampedGap * 0.15;
    }

    lastStepDistance = step;

    const roundedGap = Math.round(smoothedSegmentGap);
    return Math.max(MIN_SEGMENT_GAP, Math.min(MAX_SEGMENT_GAP, roundedGap));
  }
...
function applyBuff(type, frog) {
  ...
  function updateSingleSnake(snakeObj, dt, width, height) {
    ...
    const segmentGap = computeSegmentGap(stepDistance);
