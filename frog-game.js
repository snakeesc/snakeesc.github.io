...
function applyBuff(type, frog) {
  ...
  function updateSingleSnake(snakeObj, dt, width, height) {
    ...
    // -----------------------------
    // Path + segments follow (distance-based, with auto spacing)
    // -----------------------------
    ...
    const MAX_PATH_POINTS = 600; // tweak if needed
    ...
    const segmentCount = snakeObj.segments.length;
    if (segmentCount === 0) {
      return;
    }

    // "Ideal" pixel gap between segments
    const idealGap = isMobile
      ? SNAKE_SEGMENT_SIZE * 0.5
      : SNAKE_SEGMENT_SIZE * 0.5;

    const neededTotal = idealGap * (segmentCount + 1);

    // Measure how much path length we actually have (up to neededTotal)
    let pathLength = 0;
    for (let i = 0; i < snakeObj.path.length - 1; i++) {
      const p = snakeObj.path[i];
      const q = snakeObj.path[i + 1];
      pathLength += Math.hypot(q.x - p.x, q.y - p.y);
      if (pathLength >= neededTotal) break;
    }

    // If we don't have enough path yet (just spawned / turned), compress spacing
    let basePixelGap = idealGap;
    if (pathLength > 0 && pathLength < neededTotal) {
      basePixelGap = pathLength / (segmentCount + 1);
    }
