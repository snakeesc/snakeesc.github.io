...
// --------------------------------------------------
  ...
  function renderPauseContent(view='run',filter=pauseGuideFilter) {
    ...
    const upgradeRows=items=>items.map(x=>`<div class="pause-row">${pauseIcon(x.name)}<div><strong>${pauseEscape(x.name)}${x.count>1?' ×'+x.count:''}</strong><p>${pauseEscape((pauseGuide.find(entry=>entry[0]!=='Frogs' && entry[1].toLowerCase()===x.name.toLowerCase())||[])[2]||'Active for this run.')}</p></div></div>`).join('');
    ...
    content.innerHTML=`<section class="pause-overview"><div class="pause-score"><span>Score</span><strong>${Math.floor(score).toLocaleString()}</strong></div><div class="pause-time"><span>Time survived</span><strong>${formatTime(elapsedTime)}</strong></div></section><dl class="pause-facts"><div><dt>Frogs</dt><dd>${frogs.length}<small> / ${maxFrogsCap}</small></dd></div><div><dt>Luck</dt><dd>${luckStat}<small> / ${MAX_LUCK}</small></dd></div><div><dt>Revive chance</dt><dd>${Math.round(computeDeathRattleChanceForFrog(null)*100)}%</dd></div><div><dt>Sheds</dt><dd>${snakeShedCount}</dd></div></dl><section class="pause-kit"><h3>Current upgrades <span>${current.reduce((n,x)=>n+x.count,0)}</span></h3>${current.length?'<div class="pause-upgrade-grid">'+upgradeRows(current)+'</div>':'<p class="pause-empty">No lasting upgrades yet.</p>'}</section>${effects.length?'<section class="pause-timers"><h3>Active effects</h3><div class="pause-effect-list">'+effects.map(([name,time])=>`<span>${name} <b>${time.toFixed(1)}s</b></span>`).join('')+'</div></section>':''}`;
